...
const versionRequirements = [
  { name: 'node', currentVersion: process.version.replace('v', ''), requiredVersion: engines.node },
...
versionRequirements.forEach(({ name, currentVersion, requiredVersion }) => {
  if (!compareVersions(currentVersion, requiredVersion)) {
    console.error(`Required ${name} version: ${requiredVersion}, but found: ${currentVersion}. Use \`nvm use stable\` to switch to the correct version.`);
    process.exit(1);
  } else {
    console.log(`Current ${name} version: ${currentVersion} matches the required version: ${requiredVersion}`);
  }
});
