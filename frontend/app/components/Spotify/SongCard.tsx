...
const SongCard: React.FC<SongCardProps> = ({ track, onAdd, onRemove, isAdded }) => {
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.buttonContainer}>
        <TouchableOpacity style={styles.iconButton} onPress={isPlaying ? pauseAudio : playAudio}>
          <Ionicons name={isPlaying ? 'pause' : 'play'} size={24} color="black" />
        </TouchableOpacity>
        <TouchableOpacity style={styles.iconButton} onPress={isAdded ? onRemove : onAdd}>
          <Text style={[styles.iconText, { color: isAdded ? 'red' : 'green' }]}>{isAdded ? '-' : '+'}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
