...
function encryptMessage(message: string, recipientPublicKey: string): EncryptedMessage {
  ...
  // Encrypt the message
  const encryptedMessage = nacl.box(messageUint8Array, nonce, publicKeyUint8Array, nacl.box.keyPair().secretKey);
...
function decryptMessage(encryptedMessage: EncryptedMessage, privateKey: string): string {
  ...
  const decryptedMessage = nacl.box.open(ciphertext, nonce, nacl.box.keyPair().publicKey, privateKeyUint8Array);
